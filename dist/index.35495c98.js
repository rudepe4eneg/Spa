/* card-info v1.2.4 | Get bank logo, colors, brand and etc. by card number | https://github.com/iserdmi/card-info.git | by Sergey Dmitriev (http://srdm.io) */ !function() {
    function r(u, b) {
        r._assign(this, r._defaultProps), this.options = r._assign({}, r.defaultOptions, b || {}), this.numberSource = arguments.length ? u : "", this.number = r._getNumber(this.numberSource);
        var a = r._getBank(this.number);
        a && (this.bankAlias = a.alias, this.bankName = a.name, this.bankNameEn = a.nameEn, this.bankCountry = a.country, this.bankUrl = a.url, this.bankLogoPng = r._getLogo(this.options.banksLogosPath, a.logoPng), this.bankLogoSvg = r._getLogo(this.options.banksLogosPath, a.logoSvg), this.bankLogo = r._getLogoByPreferredExt(this.bankLogoPng, this.bankLogoSvg, this.options.preferredExt), this.bankLogoStyle = a.logoStyle, this.backgroundColor = a.backgroundColor, this.backgroundColors = a.backgroundColors, this.backgroundLightness = a.backgroundLightness, this.textColor = a.text), this.backgroundGradient = r._getGradient(this.backgroundColors, this.options.gradientDegrees);
        var s = r._getBrand(this.number);
        if (s) {
            this.brandAlias = s.alias, this.brandName = s.name;
            var n = r._getBrandLogoBasename(this.brandAlias, this.options.brandLogoPolicy, this.backgroundLightness, this.bankLogoStyle);
            this.brandLogoPng = r._getLogo(this.options.brandsLogosPath, n, "png"), this.brandLogoSvg = r._getLogo(this.options.brandsLogosPath, n, "svg"), this.brandLogo = r._getLogoByPreferredExt(this.brandLogoPng, this.brandLogoSvg, this.options.preferredExt), this.codeName = s.codeName, this.codeLength = s.codeLength, this.numberLengths = s.lengths, this.numberGaps = s.gaps;
        }
        this.numberBlocks = r._getBlocks(this.numberGaps, this.numberLengths), this.numberMask = r._getMask(this.options.maskDigitSymbol, this.options.maskDelimiterSymbol, this.numberBlocks), this.numberNice = r._getNumberNice(this.number, this.numberGaps);
    }
    r._defaultProps = {
        bankAlias: null,
        bankName: null,
        ...
        maskDelimiterSymbol: " ",
        gradientDegrees: 135
    }, r._banks = {}, r._prefixes = {}, r._brands = [
        {
            alias: "visa",
            name: "Visa",
            codeName: "CVV",
        ...
        for(var u = arguments[0], b = 1; b < arguments.length; b++){
            var a = arguments[b];
            for(var s in a)a.hasOwnProperty(s) && (a[s] instanceof Array ? u[s] = r._assign([], a[s]) : u[s] = a[s]);
        }
        return u;
    }, r._getNumber = function(r) {
        var u = r + "";
        return /^[\d ]*$/.test(u) ? u.replace(/\D/g, "") : "";
    }, r._getBank = function(r) {
        if (!(r.length < 6)) {
            var u = r.substr(0, 6);
            return this._prefixes[u] ? this._banks[this._prefixes[u]] : void 0;
        }
    }, r._getBrand = function(r) {
        for(var u = [], b = 0; b < this._brands.length; b++)this._brands[b].pattern.test(r) && u.push(this._brands[b]);
        if (1 === u.length) return u[0];
    }, r._getLogo = function(r, u, b) {
        return u ? r + (b ? u + "." + b : u) : null;
    }, r._getBrandLogoBasename = function(r, u, b, a) {
        switch(u){
            case "auto":
                return r + "-" + (a || "colored");
            case "colored":
                return r + "-colored";
            case "mono":
                return r + ("light" === b ? "-black" : "-white");
            case "black":
                return r + "-black";
            case "white":
                return r + "-white";
        }
    }, r._getLogoByPreferredExt = function(r, u, b) {
        return r || u ? r ? u ? r.substr(r.length - 3) === b ? r : u : r : u : null;
    }, r._getGradient = function(r, u) {
        return "linear-gradient(" + u + "deg, " + r.join(", ") + ")";
    }, r._getBlocks = function(r, u) {
        for(var b = u[u.length - 1], a = [], s = r.length - 1; s >= 0; s--){
            var n = b - r[s];
            b -= n, a.push(n);
        }
        return a.push(b), a.reverse();
    }, r._getMask = function(r, u, b) {
        for(var a = "", s = 0; s < b.length; s++)a += (s ? u : "") + Array(b[s] + 1).join(r);
        return a;
    }, r._getNumberNice = function(r, u) {
        for(var b = [
            0
        ].concat(u).concat([
            r.length
        ]), a = [], s = 0; b[s] < r.length; s++){
            var n = b[s], o = Math.min(b[s + 1], r.length);
            a.push(r.substring(n, o));
        }
        return a.join(" ");
    }, r._addBanks = function(r) {
        this._assign(this._banks, r);
    }, r._addPrefixes = function(r) {
        this._assign(this._prefixes, r);
    }, r.addBanksAndPrefixes = function(r) {
        this._addBanks(r.banks), this._addPrefixes(r.prefixes);
    }, r.getBanks = function(u) {
        u = r._assign({}, r.defaultOptions, u || {});
        var b = [], a = [
            "png",
            "svg"
        ], s = [
        ...
        for(var n in this._banks)if (this._banks.hasOwnProperty(n)) {
            for(var o = r._assign({}, this._banks[n]), e = 0; e < a.length; e++){
                var i = "logo" + s[e];
                o[i] && (o[i] = r._getLogo(u.banksLogosPath, o[i]));
            }
            o.backgroundGradient = r._getGradient(o.backgroundColors, u.gradientDegrees), o.logo = r._getLogoByPreferredExt(o.logoPng, o.logoSvg, u.preferredExt), b.push(o);
        }
        return b;
    }, r.getBrands = function(u) {
        u = r._assign({}, r.defaultOptions, u || {});
        for(var b = [], a = [
            "colored",
            "black",
            ...
            "Png",
            "Svg"
        ], e = 0; e < this._brands.length; e++){
            var i = r._assign({}, this._brands[e]);
            i.blocks = r._getBlocks(i.gaps, i.lengths), i.mask = r._getMask(u.maskDigitSymbol, u.maskDelimiterSymbol, i.blocks);
            for(var k = 0; k < a.length; k++){
                for(var t = "logo" + n[k], l = 0; l < s.length; l++)i[t + o[l]] = r._getLogo(u.brandsLogosPath, i.alias + "-" + a[k], s[l]);
                i[t] = r._getLogoByPreferredExt(i[t + "Png"], i[t + "Svg"], u.preferredExt);
            }
            b.push(i);
        }
        return b;
    }, r.setDefaultOptions = function(u) {
        this._assign(r.defaultOptions, u);
    }, "undefined" != typeof exports ? ("undefined" != typeof module && module.exports && (exports = module.exports = r), exports.CardInfo = r) : "undefined" != typeof window && (window.CardInfo = r);
}(), function() {
    var r = {
        "ru-absolut": {
            name: "Абсолют Банк",
    ...
    "undefined" != typeof exports ? (exports.CardInfo._banks = r, exports.CardInfo._prefixes = u) : "undefined" != typeof window && (window.CardInfo._banks = r, window.CardInfo._prefixes = u);
}();

//# sourceMappingURL=index.35495c98.js.map
